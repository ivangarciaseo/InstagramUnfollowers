...
export function getUsersForDisplay(
    ...
    for (const result of results) {
        ...
        // --- ADDED: follower count filters ---
        // Only filter users whose follower_count has been fetched (not undefined).
        // Users with undefined follower_count are always shown (they haven't been checked yet).
 ...
 * in-which we evaluate every possible status of a component's state, if we add a new
 * status to the state type, TS will complain and force us to handle it as well,
 * thus avoiding forgetting it.
 */
...
// --- ADDED: fetch follower count for a single user ---
export async function fetchFollowerCount(username: string): Promise<number | null> {
    try {
        const response = await fetch(
            ...
            {
                headers: {
                    'x-ig-app-id': '936619743392459', // Instagram Web public app ID
                },
                credentials: 'include',
            },
        );
